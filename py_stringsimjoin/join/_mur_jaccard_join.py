...
from py_stringsimjoin.utils.generic_helper import convert_dataframe_to_array, \
    ...
    split_table, get_output_header_from_tables
...
from py_stringsimjoin.utils.mur_utils import ration_output, consume_into_dataframe
from py_stringsimjoin.join.mur_set_sim_join import mur_set_sim_join
from py_stringsimjoin.utils.mur_missing_value_handler import mur_get_pairs_with_missing_value

import math

def mur_jaccard_join(ltable, rtable,
    ...
    if n_jobs <= 1:
        ...
        output_table_gen_fn = mur_set_sim_join(ltable_array, rtable_array,
        ...
        chunk_gen = ration_output(output_table_gen_fn, chunk_size)
        output_table = consume_into_dataframe(chunk_gen, header=output_header)
        ...
        for job_index in range(n_jobs):
            set_join = mur_set_sim_join(ltable_array, r_splits[job_index],
                                    l_proj_attrs, r_proj_attrs,
                                    l_key_attr, r_key_attr,
                                    l_join_attr, r_join_attr,
                                    tokenizer, 'JACCARD',
            ...
            print(job_index)
            gen = ration_output(set_join, math.floor(chunk_size/n_jobs))
            chunk_gen.append(gen)
        ...
        results = Parallel(n_jobs=n_jobs)(delayed(consume_into_dataframe)(chunk_gen[job_index],
    ...
    if allow_missing:
        missing_pairs_gen_fn = mur_get_pairs_with_missing_value(
                                        ltable, rtable,
                                        l_key_attr, r_key_attr,
                                        l_join_attr, r_join_attr,
                                        l_out_attrs, r_out_attrs,
                                        l_out_prefix, r_out_prefix,
                                        out_sim_score, show_progress)
        chunk_gen = ration_output(missing_pairs_gen_fn, chunk_size)
        missing_pairs = consume_into_dataframe(chunk_gen, header=output_header)
